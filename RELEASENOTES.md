...
- **LIFO emission order + stable ordering (SyncSignal).** `SignalOptions.Order` selects `signals.FIFO` (registration order, default) or `signals.LIFO` (reverse — most-recently-added first, the handler-stack / reverse-teardown discipline). Removal is now **order-preserving** (was swap-remove), so a sync signal's emission order is **stable across add/remove** — registration order is a real guarantee, not best-effort. `AsyncSignal` ignores `Order` (its execution order is unspecified). This makes the ordered-pipeline and handler-stack patterns first-class. *(Behaviorally, post-removal ordering was previously documented as unspecified, so this only tightens an unspecified behavior — no compile break.)*
- **`signals.StopPropagation` — short-circuit a sync chain.** An error-returning listener (`AddListenerWithErr`/`AddOnceWithErr`) on a `SyncSignal` can return `signals.StopPropagation` (or an error wrapping it) to stop the emission early: the remaining listeners are skipped. It is a **control value, not a failure** — `TryEmit` returns `nil` (not the sentinel) and `Emit` does not route it to `OnError`. Mirrors the standard library's `fs.SkipAll`. `AsyncSignal` has no sequential chain to stop, so it ignores the sentinel entirely (never joined by `TryEmit`, never routed) — a listener usable on either signal type stays well-defined.
