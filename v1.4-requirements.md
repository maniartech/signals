...
This release addresses key items from the enhancements roadmap:
- Lock-free listener management (Item 1)
- Configurable worker pools (Item 2)
- API ergonomics improvements (Item 6)
- Application-level observability (Item 7)
- Advanced testing methodologies (Item 8)

**Future versions:** See [v1.5+ Requirements](v1.5-plus.requirements.md) for planned enhancements including dynamic pool scaling and advanced metrics aggregation.
...
- Support per-signal pool size configuration
- Add basic pool utilization metrics (queue depth, active workers)
...
- Simple, predictable behavior (no complex auto-scaling)
...
#### Operational Helpers
- Simple stats API (e.g., `Stats()` returns current metrics snapshot)
- Health indicator based on error rates and queue depth
...
- **Fuzz testing**: Minimum 1 hour fuzz runs with zero crashes
...
- **Best practices**: Production usage patterns and anti-patterns
- **Performance tuning**: Guidelines for sizing worker pools, memory
...
#### Audit & Compliance
- **Deterministic behavior**: Reproducible behavior for testing
- **Versioned releases**: All releases properly tagged and versioned
- **License compliance**: Apache 2.0, no external dependencies in core
...
#### Observability
- **Instrumentation**: Metrics coverage of all operations (when enabled)
- **Local tracing**: OpenTelemetry span support for in-process tracing
- **Runtime introspection**: API to query current signal state and statistics

#### Configuration
- **Zero-downtime updates**: Library updates don't require application restarts
...
The following items are explicitly deferred to future versions (see [v1.5+ Requirements](v1.5-plus.requirements.md)):

### Deferred to v1.5
- **Dynamic Worker Pool Scaling:** Requires more research into scaling heuristics and policies
- **Advanced Metrics Aggregation:** Time-series metrics, histograms, sampling strategies
- **Performance Profiling Integration:** Continuous CPU/memory profiling capabilities
...
- **Backpressure Mechanisms:** Advanced flow control and circuit breakers

### Permanently Out of Scope (Core Library)
- **Distributed Signals:** May be separate extension package, never in core
- **Multi-datacenter Support:** Not applicable to in-process library
- **Network Communication:** Core library is in-process only

**Rationale:** v1.4 focuses on perfecting the core in-process signaling mechanism. Advanced features will be added incrementally based on production feedback and proven use cases.
...
- **Race Detection**: 100% pass rate with `-race` flag
- **Fuzz Testing**: 1-hour fuzz runs with zero crashes or hangs
- **Static Analysis**: Zero critical findings from `go vet`, `staticcheck`
- **Regression Tests**: All v1.3 benchmarks maintain or improve performance

### Enterprise Adoption Metrics

#### Production Deployment (6-Month Targets)
- **Production Usage**: 5+ production deployments (verified through feedback)
- **Scale Validation**: Proven at 100,000+ signals/sec in production
- **Stability**: Zero critical bugs reported in production
...
- **Production Guides**: Complete guides for migration, tuning, troubleshooting
- **Common Scenarios**: Documented procedures for typical operational scenarios

**Note:** Comprehensive runbooks and SLO guidelines are application-specific and not part of library documentation. See [Best Practices Guide](docs/concepts.md) for production usage patterns.

#### Security & Compliance
- **Dependencies**: Zero external dependencies in core library
- **License**: Apache 2.0
- **Security Response**: Timely response to security issues
...
- **Adoption**: Growing user base with active community engagement
- **Production Scale**: Supporting high-throughput production systems
- **Stability**: Minimal bug fixes needed, mature and stable API
- **Performance**: Competitive performance benchmarks vs alternatives
