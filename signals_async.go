...
func (s *AsyncSignal[T]) AddListenerWithCancel(handler SignalListener[T], key ...string) func() {
	s.ensureBase()
...
func (s *AsyncSignal[T]) AddOnceWithCancel(handler SignalListener[T], key ...string) func() {
	s.ensureBase()
		...
		if kl.listenerErr != nil || kl.listener != nil {
			...
			go func() {
				...
				if kl.listenerErr != nil {
					// ErrStopPropagation is a sync-only control value; async has no sequential
					...
					if err := kl.listenerErr(ctx, payload); err != nil && !errors.Is(err, ErrStopPropagation) {
						if errs != nil {
