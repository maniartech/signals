...
- **Short-Circuit**: a sync listener can return `signals.ErrStopPropagation` to halt the chain early (a control value, not a failure — like `fs.SkipAll`)
...
// ErrStopPropagation is a control value, NOT a failure — TryEmit returns nil and Emit
// does not route it to OnError. (A real error still stops AND is reported.)
OrderCreated.AddListenerWithErr(func(ctx context.Context, order Order) error {
    if order.AlreadyHandled {
        return signals.ErrStopPropagation // skip the remaining listeners, cleanly
    }
    return nil
})
...
| | [Short-Circuit Dispatch](docs/patterns/dispatch/short-circuit-dispatch.md) | Let a listener stop the chain early (return `signals.ErrStopPropagation`) so later listeners don't run — first-responder / middleware short-circuit. |
